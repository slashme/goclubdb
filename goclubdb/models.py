...
class Club(models.Model):
    ...
    meettime    = models.CharField('Meeting times of the club (as text for now)', max_length=300, null=True)
    meetplace   = models.TextField('Meeting place of the club (as text address)', null=True)
    #Adding this because some countries like Germany like to refer to clubs by postal code
    postcode    = models.CharField('Postal code', max_length=32, null=True)
    contact     = models.TextField('Contact details (Chuck all into one text field for now)', null=True)
    website     = models.URLField('Website of the club, if any', null=True)
    layer       = models.ForeignKey(Layer,      on_delete=models.CASCADE)
    clubstatus  = models.ForeignKey(Clubstatus, on_delete=models.CASCADE, null=True)
